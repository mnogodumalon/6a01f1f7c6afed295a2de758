...
import type { AndroidProjekt, CreateAndroidProjekt } from '@/types/app';
...
export class LivingAppsService {
  // --- ANDROID_PROJEKT ---
  static async getAndroidProjekt(): Promise<AndroidProjekt[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANDROID_PROJEKT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as AndroidProjekt[];
    return enrichLookupFields(records, 'android_projekt');
  }
  static async getAndroidProjektEntry(id: string): Promise<AndroidProjekt | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANDROID_PROJEKT}/records/${id}`);
    const record = { record_id: data.id, ...data } as AndroidProjekt;
    return enrichLookupFields([record], 'android_projekt')[0];
  }
  static async createAndroidProjektEntry(fields: CreateAndroidProjekt) {
    return callApi('POST', `/apps/${APP_IDS.ANDROID_PROJEKT}/records`, { fields: cleanFieldsForApi(fields as any, 'android_projekt') });
  }
  static async updateAndroidProjektEntry(id: string, fields: Partial<CreateAndroidProjekt>) {
    return callApi('PATCH', `/apps/${APP_IDS.ANDROID_PROJEKT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'android_projekt') });
  }
  static async deleteAndroidProjektEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ANDROID_PROJEKT}/records/${id}`);
  }
