...
import { AndroidProjektViewDialog } from '@/components/dialogs/AndroidProjektViewDialog';
...
const ANDROIDPROJEKT_FIELDS = [
  { key: 'paketname', label: 'Paketname', type: 'string/text' },
  { key: 'app_name', label: 'App-Name', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'version', label: 'Version', type: 'string/text' },
  { key: 'min_android_version', label: 'Minimale Android-Version', type: 'lookup/select', options: [{ key: 'android_8', label: 'Android 8.0 (Oreo)' }, { key: 'android_9', label: 'Android 9.0 (Pie)' }, { key: 'android_10', label: 'Android 10' }, { key: 'android_11', label: 'Android 11' }, { key: 'android_12', label: 'Android 12' }, { key: 'android_13', label: 'Android 13' }, { key: 'android_14', label: 'Android 14' }] },
  { key: 'programmiersprache', label: 'Programmiersprache', type: 'lookup/radio', options: [{ key: 'kotlin', label: 'Kotlin' }, { key: 'java', label: 'Java' }, { key: 'flutter', label: 'Flutter/Dart' }, { key: 'react_native', label: 'React Native' }, { key: 'sonstige', label: 'Sonstige' }] },
  { key: 'kategorien', label: 'App-Kategorien', type: 'multiplelookup/checkbox', options: [{ key: 'soziale_netzwerke', label: 'Soziale Netzwerke' }, { key: 'produktivitaet', label: 'Produktivität' }, { key: 'unterhaltung', label: 'Unterhaltung' }, { key: 'spiele', label: 'Spiele' }, { key: 'gesundheit_fitness', label: 'Gesundheit & Fitness' }, { key: 'bildung', label: 'Bildung' }, { key: 'shopping', label: 'Shopping' }, { key: 'reisen', label: 'Reisen' }, { key: 'finanzen', label: 'Finanzen' }, { key: 'sonstige_kategorie', label: 'Sonstige' }] },
  { key: 'status', label: 'Projektstatus', type: 'lookup/select', options: [{ key: 'planung', label: 'Planung' }, { key: 'in_entwicklung', label: 'In Entwicklung' }, { key: 'testing', label: 'Testing' }, { key: 'veroeffentlicht', label: 'Veröffentlicht' }, { key: 'abgeschlossen', label: 'Abgeschlossen' }] },
  { key: 'startdatum', label: 'Startdatum', type: 'date/date' },
  { key: 'geplantes_release', label: 'Geplantes Release-Datum', type: 'date/date' },
  { key: 'entwickler_vorname', label: 'Entwickler Vorname', type: 'string/text' },
  { key: 'entwickler_nachname', label: 'Entwickler Nachname', type: 'string/text' },
  { key: 'entwickler_email', label: 'Entwickler E-Mail', type: 'string/email' },
  { key: 'repository_url', label: 'Repository-URL', type: 'string/url' },
  { key: 'play_store_url', label: 'Google Play Store URL', type: 'string/url' },
  { key: 'screenshot', label: 'Screenshot / Icon', type: 'file' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'android_projekt', label: 'Android Projekt', pascal: 'AndroidProjekt' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('android_projekt');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'android_projekt': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'android_projekt': return (data as any).androidProjekt as AndroidProjekt[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'android_projekt': return ANDROIDPROJEKT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'android_projekt': return {
        create: (fields: any) => LivingAppsService.createAndroidProjektEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAndroidProjektEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAndroidProjektEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'android_projekt' || dialogState?.entity === 'android_projekt') && (
        <AndroidProjektDialog
          open={createEntity === 'android_projekt' || dialogState?.entity === 'android_projekt'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'android_projekt' ? handleUpdate : (fields: any) => handleCreate('android_projekt', fields)}
          defaultValues={dialogState?.entity === 'android_projekt' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['AndroidProjekt']}
          enablePhotoLocation={AI_PHOTO_LOCATION['AndroidProjekt']}
        />
      )}
      {viewState?.entity === 'android_projekt' && (
        <AndroidProjektViewDialog
          open={viewState?.entity === 'android_projekt'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'android_projekt', record: r }); }}
        />
      )}
